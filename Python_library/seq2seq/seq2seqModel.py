    ...
    def train_epoch(self, training_generator, criterion, criterionDicrim, accDiscr, bornInf, bornSup, epoch, args):
        ...
        for local_batch, local_labels, local_key, local_beat in training_generator:
            if args.alphaRep == "alphaRep":
                local_batch = nn.functional.one_hot(local_batch.long(),args.n_categories).float()           
                local_labels = nn.functional.one_hot(local_labels.long(),args.n_categories).float()
